...
for sched in "FRFCFS" "FCFS" "ATLAS" "PARBS" "TCM" "BLISS" "TCMPlus"
# for sched in "MLQSched"
do
# sched=$2
    for core in 4 8 16 32
    do  
        ### Run with 10 million instructions
        ### Uncomment the line below in order to generate configuration files
        generateConfigs $core $sched 30000000
        ...
        do  
            if [ ${core} -eq 1 ];
            then
                mono bin/sim.exe -output ${outdir}/${core}core_work${i}_FRFCFS_.json -config configs/${core}core_base_$sched.cfg -N $core -workload workloads/${core}core $i
            else
                mono bin/sim.exe -output ${outdir}/${core}core_work${i}_${sched}_.json -config configs/${core}core_base_$sched.cfg -N $core -workload workloads/${core}core $i
            fi
            sleep 5
        done
    done
